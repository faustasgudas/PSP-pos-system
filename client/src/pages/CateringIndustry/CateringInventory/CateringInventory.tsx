...
import "../../BeautyIndustry/BeautyInventory/BeautyInventory.css";
...
export default function CateringInventory() {
    ...
    const rows = useMemo(() => {
        ...
        return activeProducts
            ...
            .filter(({ product }) => {
                if (!q) return true;
                return String(product.name).toLowerCase().includes(q) || String(product.code).toLowerCase().includes(q);
            })
    ...
    const load = async () => {
        ...
        setError(null);
        try {
            const [items, prods] = await Promise.all([listStockItems(businessId), listCatalogItems(businessId, { type: "Product" })]);
        ...
        <div className="inventory-container">
            <div className="action-bar">
                ...
                <div className="action-bar__right">
                    <input className="inventory-search" placeholder="Search products…" value={query} onChange={(e) => setQuery(e.target.value)} disabled={loading} />
                        ...
                        {!loading &&
                            rows.map(({ product, stock }) => (
                                <tr key={product.catalogItemId} className="inventory-row">
                                    ...
                                    <td className="right">
                                        {stock ? <span className="stock-pill">{stock.qtyOnHand} {stock.unit}</span> : <span className="muted">Not tracked</span>}
                                    </td>

                                    <td className="right">
                                        {canManage && (
                                            <button className="btn btn-ghost" type="button" onClick={() => openManage(product)} disabled={saving}>
                                                Manage
                                            </button>
                                        )}
                                    </td>
                        ...
                        <div className="modal-form">
                            ...
                            <div className="modal-field">
                                <label style={{ display: "flex", gap: 10, alignItems: "center" }}>
                                    <input type="checkbox" checked={createTrackStock} onChange={(e) => setCreateTrackStock(e.target.checked)} disabled={saving} />
                            ...
                            {createTrackStock && (
                                <>
                                    <div className="modal-field">
                                        <label>Unit</label>
                                        <BeautySelect value={createUnit} onChange={(v) => setCreateUnit(v as any)} disabled={saving} options={UNITS.map((u) => ({ value: u, label: u }))} />
                                    </div>
                                        ...
                                        <div className="modal-field">
                                            ...
                                            <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
                                                <div>
                                                    ...
                                                    <BeautySelect value={editEnableUnit} onChange={(v) => setEditEnableUnit(v as any)} disabled={saving} options={UNITS.map((u) => ({ value: u, label: u }))} />
                                                </div>

                                                <input placeholder="Initial qty" type="number" value={editEnableQty} onChange={(e) => setEditEnableQty(e.target.value)} disabled={saving} />
                                ...
                                return (
                                    <>
                                        <div className="modal-field">
                                            ...
                                            <BeautySelect value={editUnit} onChange={(v) => setEditUnit(v as any)} disabled={saving} options={UNITS.map((u) => ({ value: u, label: u }))} />
                                        </div>

                                        <div className="modal-field">
                                            <label>Set stock to</label>
                                            <div style={{ display: "grid", gap: 10 }}>
                                                <input type="number" placeholder="Target qty" value={editSetToQty} onChange={(e) => setEditSetToQty(e.target.value)} disabled={saving} />
                                                <input type="number" placeholder="Unit cost snapshot (optional)" value={editSetUnitCost} onChange={(e) => setEditSetUnitCost(e.target.value)} disabled={saving} />
                                        ...
                                        <div className="modal-field">
                                            ...
                                            <div style={{ display: "grid", gap: 10 }}>
                                                <input type="number" placeholder="Receive qty" value={editReceiveQty} onChange={(e) => setEditReceiveQty(e.target.value)} disabled={saving} />
                                                <input type="number" placeholder="Unit cost (required)" value={editReceiveUnitCost} onChange={(e) => setEditReceiveUnitCost(e.target.value)} disabled={saving} />
